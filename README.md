...
The detection is structurally guaranteed: postcode area encodes diversity score by construction, and CatBoost is powerful enough to recover that encoding from 20,000 policies every time. Spearman is not — the relationship is non-linear and categorical, so rank correlation cannot detect it.

| | Library (proxy R2) | Manual (Spearman) |
|---|---|---|
| Detection rate (50 seeds) | 50/50 (100%) | 0/50 (0%) |
| Mean score | R2 = 0.769 ± 0.008 | mean abs(r) = 0.063 |
| Threshold | > 0.10 | > 0.25 |
| Result | postcode_area RED | all clear |
