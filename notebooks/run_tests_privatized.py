"""
Run the full insurance-fairness test suite on Databricks serverless.

Usage (from repo root):
    python notebooks/run_tests_privatized.py
"""
import os
import sys
import time
...
# Load Databricks credentials from env file
...
# Key: copy test files to /tmp (writable) before running pytest there.
NOTEBOOK_CONTENT = '''
...
# Step 2: install package from workspace
rc2 = run(
...
# Step 3: copy tests to /tmp so pytest can write pycache
test_src = "/Workspace/insurance-fairness/tests"
test_dst = "/tmp/insurance_fairness_tests"
if os.path.exists(test_dst):
    shutil.rmtree(test_dst)
shutil.copytree(test_src, test_dst)
lines.append(f"Copied tests to {test_dst}")

# Step 4: run pytest from /tmp
env = dict(os.environ)
...
notebook_path = f"{WORKSPACE_DIR}/run_pytest"
...
run_resp = w.jobs.submit(
    run_name="insurance-fairness-privatized-audit-tests",
    tasks=[
        jobs.SubmitTask(
            task_key="run-pytest",
...
if task_run_id:
    try:
        ...
        if output.notebook_output and output.notebook_output.result:
            print("\n--- Test output ---")
            print(output.notebook_output.result[:15000])
        if output.error:
