# Databricks notebook source
# MAGIC %pip install git+https://github.com/burningcost/insurance-fairness.git catboost polars scikit-learn scipy jinja2 pyarrow pytest

# COMMAND ----------

import subprocess, sys, os, tempfile, re

work_dir = tempfile.mkdtemp(prefix="insurance_fairness_")
repo_url = "https://github.com/burningcost/insurance-fairness.git"
clone = subprocess.run(["git", "clone", repo_url, work_dir], capture_output=True, text=True, timeout=120)
if clone.returncode != 0:
    dbutils.notebook.exit("CLONE_FAILED: " + clone.stderr[-1000:])

result = subprocess.run(
    [sys.executable, "-m", "pytest", os.path.join(work_dir, "tests"), "-v", "--tb=short"],
    capture_output=True, text=True, timeout=300, cwd=work_dir,
)

output = result.stdout + result.stderr
clean_output = re.sub(r'\x1b\[[0-9;]*m', '', output)

# Write to UC volume (available in serverless)
# Try writing to a catalog volume - but first just exit with the output
exit_msg = f"EXIT_CODE:{result.returncode}\n" + clean_output[-5000:]

# dbutils.notebook.exit can accept a string up to 1MB
dbutils.notebook.exit(exit_msg)
